...
def train_category_model(category_name, features, data, cluster_descriptions):
    ...
    # 🔥 NEW: Use skill-specific features for clustering
    skill_cluster_features = category_features.get(category_name, base_features)
    ...
    print(f"Using {len(skill_cluster_features)} skill-specific clustering features")
    ...
    for fold_idx, (train_index, test_index) in enumerate(skf.split(X, y), 1):
        ...
        # ========== 🔥 SKILL-SPECIFIC CLUSTERING (NEW) ==========
        scaler_c, pca, kmeans, valid_cluster_feats = fit_skill_specific_cluster_model(
            X_train, y_train, 
            category_name,           # ← Skill name
            skill_cluster_features,  # ← Skill-specific features
            n_clusters=5
        )

        # Add cluster features
        X_train = add_skill_cluster_features(
            X_train, y_train, scaler_c, pca, kmeans, valid_cluster_feats, is_train=True
        )
        X_test = add_skill_cluster_features(
            X_test, y_test, scaler_c, pca, kmeans, valid_cluster_feats, is_train=False
        )

        # Visualize clusters (only once per category)
        if fold_idx == 1:
            viz_folder = os.path.join(OUTPUT_FOLDER, 'skill_cluster_visualizations')
            os.makedirs(viz_folder, exist_ok=True)
            visualize_skill_clusters(
                X_train, y_train, scaler_c, pca, kmeans,
                valid_cluster_feats, category_name, viz_folder
            )
        # ========================================================
        ...
        # --- Train Decision Tree Ensemble ---
        print("\nTraining Decision Tree Ensemble...")
        model_results = train_and_evaluate_models(
            X_train_res, X_test_scaled, y_train_res, y_test, category_name, class_weight_dict
        )
    ...
    # ----- Report skill-specific cluster distributions -----
    print("\n✅ Skill-specific clustering complete for", category_name)
    print("Cluster visualizations saved to OUTPUT_FOLDER/skill_cluster_visualizations/")

    return best_fold_results, test_results

def fit_skill_specific_cluster_model(X_train, y_train, skill_name, skill_features, n_clusters=5, random_state=42):
    """
    Cluster players based on skill-specific college stats + NBA outcome.
    This captures "success patterns" rather than just "player types".
    
    Args:
        X_train: Training features (DataFrame)
        y_train: Training labels (Series) - NBA outcome (0, 1, 2)
        skill_name: Name of skill being clustered (e.g., 'Rebounding')
        skill_features: List of features relevant to this skill
        n_clusters: Number of clusters
        random_state: Random seed
    
    Returns:
        scaler, pca, kmeans: Fitted models for transforming test data
    """
    print(f"\n🔬 Performing skill-specific clustering for {skill_name}...")
    
    # Get only skill-relevant features that exist in X_train
    valid_features = [f for f in skill_features if f in X_train.columns]
    
    if len(valid_features) < 3:
        print(f"⚠️ Only {len(valid_features)} valid features for {skill_name}. Using all available.")
        valid_features = X_train.columns.tolist()[:10]
    
    print(f"   Using {len(valid_features)} features: {', '.join(valid_features[:5])}...")
    
    # Combine features with NBA outcome label for clustering
    cluster_data = X_train[valid_features].copy()
    cluster_data['nba_outcome'] = y_train.values  # Add NBA outcome (0, 1, 2)
    
    # Scale all features including the label
    scaler = StandardScaler()
    scaled_data = scaler.fit_transform(cluster_data)
    
    # PCA for dimensionality reduction
    n_components = min(5, len(valid_features) + 1)  # +1 for the label
    pca = PCA(n_components=n_components, random_state=random_state)
    pca_data = pca.fit_transform(scaled_data)
    ...
    print(f"\n   📊 Cluster Distribution for {skill_name}:")
    for c in range(n_clusters):
        ...
        pct_top = (cluster_labels == 2).sum() / len(cluster_labels) * 100
        pct_mid = (cluster_labels == 1).sum() / len(cluster_labels) * 100
        pct_bot = (cluster_labels == 0).sum() / len(cluster_labels) * 100
        
        print(f"      Cluster {c} (n={len(cluster_labels)}): "
              f"Top={pct_top:.0f}%, Mid={pct_mid:.0f}%, Bot={pct_bot:.0f}%")
    
    return scaler, pca, kmeans, valid_features


def add_skill_cluster_features(X, y, scaler, pca, kmeans, valid_features, is_train=True):
    """
    Add cluster features using the fitted skill-specific clustering model.
    
    Args:
        X: Features (train or test)
        y: Labels (train or test) - can be None for test if not using label
        scaler, pca, kmeans: Fitted clustering models
        valid_features: Features used for clustering
        is_train: Whether this is training data (used for visualization logic)
    
    Returns:
        X with added cluster features
    """
    X = X.copy()
    
    # Prepare cluster data (features only for test, features + label for train)
    cluster_data = X[valid_features].copy()
    
    if is_train and y is not None:
        # For training: include label in clustering (as we did during fit)
        cluster_data['nba_outcome'] = y.values
    else:
        # For test: use a dummy label (won't affect cluster assignment much)
        # Alternatively, use predicted probabilities, but that causes leakage
        cluster_data['nba_outcome'] = 1  # Middle category as default
...
def visualize_skill_clusters(X_train, y_train, scaler, pca, kmeans, valid_features, skill_name, output_folder):
    """
    Create visualization of skill-specific clusters in PCA space.
    """
    # Prepare data for visualization
    cluster_data = X_train[valid_features].copy()
    cluster_data['nba_outcome'] = y_train.values
    ...
    # Color by cluster, shape by NBA outcome
    markers = {0: 'v', 1: 'o', 2: '^'}  # Bottom, Middle, Top
    ...
    for c in range(kmeans.n_clusters):
        ...
        for outcome in [0, 1, 2]:
            outcome_mask = cluster_y == outcome
            if outcome_mask.sum() == 0:
                continue
                
            outcome_pca = cluster_pca[outcome_mask.values]
            label = f"Cluster {c} - {'Top' if outcome == 2 else 'Mid' if outcome == 1 else 'Bot'}"
            
            plt.scatter(
                outcome_pca[:, 0], outcome_pca[:, 1],
                c=[colors[c]], marker=markers[outcome],
                s=60, alpha=0.6, label=label, edgecolors='black', linewidths=0.5
            )
    ...
    plt.title(f'{skill_name} Clusters (Skill-Specific + NBA Outcome)\nv=Bottom, o=Middle, ^=Top')
    plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left', fontsize=8)
    plt.grid(True, alpha=0.25)
    plt.tight_layout()
    
    viz_path = os.path.join(output_folder, f'{skill_name.lower()}_skill_clusters.png')
    plt.savefig(viz_path, dpi=200, bbox_inches='tight')
    plt.close()
    print(f"   💾 Saved {skill_name} cluster visualization: {viz_path}")

def prepare_data():
    print("Loading and preparing data...")
    nba_data = pd.read_csv('player_success_scores.csv')
